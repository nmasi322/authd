...
async function initialise() {
    ...
    const { customCommand } = Frameworks.flatMap((framework) => framework.variants).find((variant) => variant.name === template) ?? {};
    if (customCommand) {
    ...
    const templateDir = path.resolve(fileURLToPath(import.meta.url), "../../", `template-${template.toLowerCase()}`);
