...
class AuthService {
  async register(data) {
    if (!data.username) throw new CustomError("name is required", 400);
  ...
  async refreshAccessToken(data) {
    ...
    const accessToken = jsonwebtoken.sign({ id: user.id }, JWT.JWT_SECRET, {
      expiresIn: "1h",
    });
  ...
  async resetPassword(data) {
    ...
    User.findOne({ where: { id: RToken.user } })
      .then((user) => {
